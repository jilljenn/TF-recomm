...
for filename in experiments:
    ...
    for ax_id, category_regexp in enumerate(categories_regexp):
        if category_regexp in labels[filename] + ' ':
            curve, = acc[ax_id].plot(range(1, nb_epochs + 1), acc_values[filename], label=labels[filename])
            acc_curves[ax_id].append(curve)
            curve, = nll[ax_id].plot(range(1, nb_epochs + 1), nll_values[filename], label=labels[filename])
